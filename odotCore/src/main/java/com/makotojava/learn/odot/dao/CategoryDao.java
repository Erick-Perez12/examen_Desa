...
public class CategoryDao {
  ...
  public Category findByDescription(String description) {
	    Category ret = null;
	    String sql = "SELECT * FROM " + Category.TABLE_NAME + " WHERE description = ?";
	    Object[] paramValues = { description };
	    JdbcTemplate jdbc = new JdbcTemplate(getDataSource());
	    List<Category> categories = jdbc.query(sql, paramValues, new CategoryRowMapper());
	    if (categories.size() > 1) {
	      throw new RuntimeException("Expected 1 result from findByDescription(), instead found " + categories.size()
	          + " (DB configuration error, maybe?)");
	    }
	    if (!categories.isEmpty()) {
	      ret = categories.get(0);
